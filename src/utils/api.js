// src/utils/api.js
// Shared API module: static data-file base URL, ESI base, and retry logic

// Preferred data source: same-origin static files under /data (published to gh-pages by
// the "Refresh market data" GitHub Action; see data/region_orders/<regionId>.json)
    ...
    try {
        const tryParse = (href) => {
            ...
            if (host.endsWith('github.io') && pathParts.length > 0) {
                ...
                return { owner: sub, repo: pathParts[0] };
            }
            return null;
        };
        ...
        // Not served from a <owner>.github.io/<repo>/ URL (e.g. local dev) — unknown.
        return null;
    } catch {
        return null;
    }
...
export async function fetchDataLastCommitTime() {
    const info = detectRepoInfoFromEnvironment();
    if (!info) return null;
